...
​		Критическая секция для n потоков. Как он приблизительно выглядит для 2х потоков

```c#
public class Mutex
{
    public volatile bool[2] capture;
...
```c#
bool CompareAndSet(текущее значение addr, предполагаемое значение old, новое значение new)
{
    if ( addr != old )
    	return false;
  	addr = new;  // Здесь что-то может пойти не так
  	return true;
}  
  
```

Фишка в том, что на уровне процессора это атомарная инструкция *cmpxchg*

Тогда, например, удаление элемента, это просто вызов *CAS(pred.next, curr, curr.next)*

**!!!** На *CAS* можно построить любой lock-free алгоритм, теоретически доказано.



## 5. Шаблоны || программирования

#### 5.1. Типичные ошибки || программирования

**DeadLock**. Поток A  делает a.lock(); b.lock(); Поток B делает b.lock(); a.lock(); В итоге бесконечно ждут друг друга. Есть программы, которые пишут в журнал все блокировки, постфактум можно понять, что у нас случился deadlock.

Если создать Fork процесса, то не очень понятно что произойдет с блокировками и лучше так не делать.

**DataRace** [32 32] mov mov Выявить -  не решаемая алгоритмически задача.

**ABA проблема**  Если из списка удалить элемент, уничтожить его, а затем создать новый элемент и добавить обратно в список, есть вероятность, что новый элемент будет размещён на месте старого. Указатель на новый элемент совпадёт с указателем на старый, что и приведёт к проблеме: равенство признаков не есть равенство данных целиком. Очевидным образом отсутствует в языках с управляемой памятью.



#### 5.2. Порядок действий

1. А нужно ли || ? Может процессор помощнее?
2. Находим место ||
3. Выбираем алгоритм
4. Выбираем структуру
5. Параллельности по задачам или данным?

По данным это когда у каждого свой кусок, по задачам это конвейер, например,  System.Threading.Tasks.Dataflow.



## 6. Казалось бы, причем тут это

#### 6.1. Условная переменная 

Примитив синхронизации, обеспечивающий блокирование одного или нескольких потоков до момента поступления уведомления от другого потока о выполнении некоторого условия или до истечения максимального промежутка времени ожидания.  Их используют брокеры сообщений.

#### 6.2. Атомарные снимки регистров

I -> x1 II-> x2, как получить снимок на какой-то момент времени? Добавляй переменную версии и сравнивай ее!

#### 6.3. Шаблон DoubleCheck
...
В настольном пользовательском интерфейсе есть только 1 поток, который работает с UI компонентами и его не желательно блокировать